...
The `apt` package manager commonly used in Ubuntu maintains the list of sources of packages in `/etc/apt/sources.list`. When you are using a fresh PC, you do not have [internet running on the terminal](internet_on_my_terminal.md). Therefore, packages from Ubuntu's standard repositories are not available.
...
You can also use packages from the internet once you have internet set up on your terminal. Refer to [Running internet from the Terminal](internet_on_my_terminal.md)
